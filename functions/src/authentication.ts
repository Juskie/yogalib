...
export const sendMailConfirmation = functions.https.onCall(
	(data: { email: string }) => {
		const mailOptions = {
			from: `camille.maisonobe@gmail.com`,
			to: data.email,
...
export const newUserSignUp = functions.https.onCall(
	async (data: {
		email: string;
		password: string;
		firstName: string;
...
export const checkUserEmail = functions.https.onCall(
