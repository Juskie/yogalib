...
class SignUp extends Component {
	...
	handleChange = (event) => {
		...
		switch (name) {
			case "firstName":
				errors.firstName = validFormErrorRegex.firstName.test(value)
					? ""
					: "Prénom obligatoire";
				break;
			case "lastName":
				errors.lastName = validFormErrorRegex.lastName.test(value)
					? ""
					: "Nom obligatoire";
				break;
			case "email":
				errors.email = validFormErrorRegex.email.test(value)
					? ""
					: "Email obligatoire";
				break;
			case "phone":
				errors.phone = validFormErrorRegex.phone.test(value)
					? ""
					: "Format du téléphone incorrect";
				break;
			case "password":
				errors.password = validFormErrorRegex.password.test(value)
					? ""
					: "Mot de passe obligatoire";
				break;
			case "confirmPassword":
				errors.confirmPassword =
					value.length > 0 ? "" : "Confirmer votre Mot de Passe";
				break;
			default:
				break;
		}
	...
	handleSubmit = async (event) => {
		...
		const sendMailConfirmation = firebase
			.functions()
			.httpsCallable("sendMailConfirmation");
			...
			<>
				<section className="signUpContainer">
					<form className="form" onSubmit={this.handleSubmit}>
						...
						{submitPassword && <p>{submitPassword}</p>}
						<label>
							<input type="checkbox" id="cgu" />
							J'ai lu et j'accepte les CGU
						</label>
						{authError && <p>{authError}</p>}
						{validFormError.length > 0 && (
							<span className="error">{validFormError}</span>
						)}
